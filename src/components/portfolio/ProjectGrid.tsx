      ...
      <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          ...
          >
            ...
            {item.subtitle && (
              <p className="mt-1 text-sm text-accent-2">{item.subtitle}</p>
            )}
            ...
            {item.external_url && (
              <a
                ...
                className="mt-4 inline-flex w-fit items-center gap-1 text-sm font-semibold text-accent-2 hover:underline"
              >
                {visitLabel} →
              </a>
            )}
